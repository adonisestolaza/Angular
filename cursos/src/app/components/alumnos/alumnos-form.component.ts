...
export class AlumnosFormComponent extends CommonFormComponent<Alumno, AlumnoService> implements OnInit {
  ...
  public selecionarFoto(event): void {
  ...
  public crear(): void {
    ...
    if (!this.fotoSeleccionada) {
      ...
      this.service.crearConFoto(this.model, this.fotoSeleccionada).subscribe(m => {
  ...
  public editar(): void {
    ...
    if (!this.fotoSeleccionada) {
      ...
      this.service.editarConFoto(this.model, this.fotoSeleccionada).subscribe(m => {
        ...
        console.log(m);
        Swal.fire('Modificado:', `${this.nombreModel} ${m.nombre} modificado con Exito!`, 'success')
        this.router.navigate([this.redirect])
      }, err => {
        if (err.status === 400) {
          this.error = err.error;
